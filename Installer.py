# Author:   Ronoaldo JLP 
# Date:     May 24, 2020
# Description:  Install printer definition and other scripts for the Flashforge CreatorPro printer.
# License:  GPLv3
...
class Installer(Extension):
    ...
    def __init__(self):
        super().__init__()
        self.setMenuName("Flashforge CreatorPro")
    ...
    def installFiles(self, showMessage=True):
        ...
        plugin_path = os.path.join(Resources.getStoragePath(Resources.Resources),
                "plugins", "FlashforgeCreatorproIntegration", "FlashforgeCreatorproIntegration")
        ...
        resource_map = {
            ...
            "creatorpro_extruder_base.def.json": {
                "src": os.path.join(plugin_path, "printer", "defs"),
                "dst": os.path.join(definitions_path)
            },
             "creatorpro_extruder_1.def.json": {
                "src": os.path.join(plugin_path, "printer", "extruders"),
                "dst": os.path.join(resources_path, "extruders")
            },
            "creatorpro_extruder_2.def.json": {
                "src": os.path.join(plugin_path, "printer", "extruders"),
                "dst": os.path.join(resources_path, "extruders")
            },
             "FlashForge_CreatorPro.stl": {
        ...
        if showMessage:
            if restart_required:
                msg = catalog.i18nc("@info:status", "Flashforge Creatorpro files installed. Please restart Cura.")
            else:
                msg = catalog.i18nc("@info:status", "Flashforge Creatorpro files were already installed.")
            Message(msg).show()
